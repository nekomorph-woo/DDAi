...
读取 `~/.claude/wok/resolve-system-name.md` 执行解析。用户输入作为参数（可以是 system-name、缩写、或 system-name + phase）。

解析结果为 `<phase-dir>`，即 `.wok-plans/` 下的相对路径：

- 扁平系统：`<phase-dir>` = `fix-auth-token-expire`
- 多阶段系统：`<phase-dir>` = `feat-smart-home-system/p1-core-device`

若输出为 `PHASES:` + 多行 → 用 AskUserQuestion 让用户选择 phase 后重新调用脚本。
若输出为 `AMBIGUOUS:` → 用 AskUserQuestion 让用户选择后重新调用脚本。

### 2. 验证前置

- `.wok-plans/<phase-dir>/_plan.md` 不存在 → 报错退出
- 所有 ACTION step 已 `[x]` → 跳过实现，直接检查 review 和验收标准
- 初始化 `.wok-plans/<phase-dir>/_autopilot.md`（不存在则创建）
...
1. 将 `plugins/wok/scripts/track-changes.py` 复制到 `.wok-plans/<phase-dir>/.hooks/`
...
**路径约定**：本 agent 所有路径基于 `<phase-dir>` 变量，由 resolve-system-name.sh 解析得到：

- `.wok-plans/<phase-dir>/_plan.md` — 执行计划
- `.wok-plans/<phase-dir>/_review.md` — 审查报告
- `.wok-plans/<phase-dir>/_autopilot.md` — 执行日志
- `.wok-plans/<phase-dir>/_define.md` — 需求定义（验收标准来源）
- `.wok-plans/<phase-dir>/.hooks/` — 临时 hooks 目录

**关键：每完成一个子步骤后立即写日志到 `_autopilot.md`，DO NOT 批量在最后写入。**

```
while (收敛条件未满足) {
    读 .wok-plans/<phase-dir>/_plan.md → 解析 step groups
    // step 识别规则：
...
2. 读 `.wok-plans/<phase-dir>/_plan.md` → 定位当前 `[ ]` ACTION step
3. **从 plan 提取任务内容**（不传 plan 路径给 impl）：
   - step 标题和描述
   - 涉及的文件列表
   - 要实现的接口列表
   - 验证标准
   - 覆盖验收标准：从 step 的 `**覆盖验收标准**` 字段提取编号，再从 `.wok-plans/<phase-dir>/_define.md` 的 `## 验收标准` section 读取对应文字
   - 测试覆盖声明：从 step 的 `**测试覆盖**` 字段提取三层（正常路径/边界条件/异常路径）文字
...
5. 等待 subagent 完成
6. 验证完成（按环境选择）：
   - **Claude Code**：读 `.wok-plans/<phase-dir>/.hooks/changes.log`，检查当前 step 涉及的文件路径是否出现在日志中。每个 group 首个 ACTION step 前清空 `changes.log`
   - **Cursor**：`git diff --stat HEAD`，对比 baseline 确认变更
7. subagent 报告 `⚠️ 阻塞` → 在 `.wok-plans/<phase-dir>/_plan.md` 该 step 标记 `⚠️ 阻塞` → 🛑 中断
...
2. 等待 CR 完成（检测 + 分析 + 报告）
3. 读 `.wok-plans/<phase-dir>/_review.md` 检查结果：
...
while (有 🔴🟠 且 round < max-fix-rounds) {
    ...
    1. 读 `.wok-plans/<phase-dir>/_review.md` 提取 🔴🟠 findings 的洞察分析
    ...
    6. 读 `.wok-plans/<phase-dir>/_review.md` 检查收敛
    7. 收敛 → 批量回填
    8. 未收敛 → 继续下一轮
}
```
...
1. 读取 `.wok-plans/<phase-dir>/_plan.md`
2. 定位当前 group 的 step 区间
3. 将区间内所有 `[ ]` 替换为 `[x]`
4. 写回 `.wok-plans/<phase-dir>/_plan.md`
5. 记录到 `.wok-plans/<phase-dir>/_autopilot.md`

### E. 验收标准

所有 group 完成后，检查验收标准：

1. 读取 `.wok-plans/<phase-dir>/_define.md` 或 `.wok-plans/<phase-dir>/_issue.md` 中的 `## 验收标准`
...
每个关键节点写入 `.wok-plans/<phase-dir>/_autopilot.md`：
...
1. 写 🛑 日志到 `.wok-plans/<phase-dir>/_autopilot.md`：
...
重新运行 `claude --agent wok-autopilot "<input>"`（input 可以是 system-name、缩写、或 phase）：

1. 读 `.wok-plans/<phase-dir>/_autopilot.md` 尾部 → 找最后 🛑 或 ✅ 条目
2. 读 `.wok-plans/<phase-dir>/_plan.md` → 确认哪些 step 还是 `[ ]`
3. 读 `.wok-plans/<phase-dir>/_review.md` → 确认收敛状态
4. 重新部署 hooks（CC 环境）
5. 写恢复记录并继续循环
...
1. 复制 `plugins/wok/scripts/track-changes.py` 到 `.wok-plans/<phase-dir>/.hooks/track-changes.py`
2. 清空旧的 `changes.log`（如存在）
3. hooks 配置注入（通过 settings merge 或直接配置）

### 使用

每个 ACTION step 完成后，读取 `.wok-plans/<phase-dir>/.hooks/changes.log` 验证文件变更。

### 清理

autopilot 结束时（完成或 handoff）：

1. 删除 `.wok-plans/<phase-dir>/.hooks/` 目录
2. 删除 hooks 配置
