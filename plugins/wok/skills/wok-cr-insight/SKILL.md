...
2. **根因追溯**（找出导致问题的根本原因，而不只是表象）：分类为以下之一
   - 设计缺陷：需求阶段遗漏或设计不充分
   - 编码疏忽：实现过程中的疏忽或简化
   - 架构约束：现有架构导致的最优妥协
   - 上下文缺失：缺少领域知识或业务背景
3. **修改方案**：给出可直接执行的代码，DO NOT 给出伪代码
4. **一致性评估**（仅管道模式；指核对修复是否偏离需求文档的既定方向）：对照 `_define.md` 的设计锚点（方向约束）评估

### Step 4: 追加写入

> **📝 文风要求**：三个引用块的内容都用完整句子写，不写电报体。完整规范见 `skills/_shared/doc-writing-style.md`。

在每个 finding 下方追加 3 个引用块：

```markdown
> **🔍 原因分析**
> <根因分类>: <用完整句子写清根本原因。根因分类为以下之一：设计缺陷（需求阶段遗漏）/ 编码疏忽（实现时疏忽）/ 架构约束（现有架构导致）/ 上下文缺失（缺领域知识）。如"编码疏忽：开发者假设数据库里所有用户都有盐值，未处理 salt 为空的情况">

> **🔧 修改方案**
> <用完整句子写出修改步骤并附完整代码，如"在 hashUtil.verify 函数入口加入盐值空值检查，空值时抛出 InvalidCredentialException">

> **📐 一致性评估**
> <用完整句子说明修复与需求文档/设计目标的关系，如"修复符合 _define.md 中 [SECURITY] 锚点'密码校验必须安全'的要求，不偏离原设计">
```
