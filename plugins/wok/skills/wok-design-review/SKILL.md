...
> 1. 写完整句子——问题数/阻塞等概要用完整句而非数字堆砌；finding 描述说清"现象 + 影响"
> 2. 术语首次出现就通俗解释（接口一致性、依赖方向、跨模块遗漏等见检查项说明）
> 3. 每个检查维度开头加 1 句"这一项在查什么"

> **检查项**：4 项（接口一致性 / 依赖方向 / 跨模块遗漏 / 输出格式）
> **问题数**：共发现 <N> 个问题——🔴 <N> 个阻塞、🟡 <N> 个建议、🟢 <N> 个已通过
> **阻塞**：<用完整句说明，如"无阻塞项"或"有 2 个 🔴 阻塞项，需先解决才能继续">

## 检查结果

### 接口一致性

🟡 **Finding 1: <用一句话写清问题，如"auth 模块调用了 payment 模块尚未定义的方法">**

<用 2-3 句完整话说明来龙去脉：哪个模块的哪个接口与另一个模块的定义不对齐，具体差在哪里>

- **影响模块**：<列出受影响的模块，如"auth-core、api-gateway">
- **修复**：<用完整句子给修复建议，如"在 payment/design.md 中补齐 charge 方法的接口定义，auth-core 据此调整调用">

#### [OPEN] 🟡-1: <对应行动项标题，如"补齐 payment.charge 接口定义">

- [ACTION] <用完整句子写清动作，如"编辑 modules/payment/design.md，在接口契约中增加 charge 方法签名">

🟢 **Finding 2: <用一句话写清通过项，如"auth 与 user 模块的依赖方向正确">**

<用 1-2 句完整话说明为什么这一项通过>
