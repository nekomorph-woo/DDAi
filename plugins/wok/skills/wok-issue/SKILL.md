...
创建有序的 RED-GREEN 循环列表（TDD 三阶段：先写测试看到它失败 RED，再写最小代码让它通过 GREEN，最后重构 REFACTOR）。每个循环是一个垂直切片——即一次完成"一个行为的测试 + 对应实现"、端到端可验证，而不是先写完所有测试再写所有实现：
...
> 1. 写完整句子——"实际行为/预期行为/复现步骤"都用完整句描述，不写电报体
> 2. 术语首次出现就通俗解释（回归缺陷、RED-GREEN、垂直切片等见下方）
> 3. TDD 修复计划的每个 RED/GREEN 写清"测什么行为 / 改什么代码"，不写空占位符
...
- **实际行为**：<用完整句描述实际异常，如"用户用正确密码登录时，接口返回 500，日志显示 NullPointerException">
- **预期行为**：<用完整句描述应该发生的行为，如"用户用正确密码登录应返回 200 和有效 JWT">
- **复现步骤**：<用完整句写出重现步骤，如"1. 启动本地服务；2. 用测试账号 POST /api/login；3. 观察返回 500">

## 根因分析

- **代码路径**：<用完整句描述问题代码的调用链，如"login service 校验密码时调用 hashUtil.verify，后者在盐值为空时抛 NPE">
- **失败原因**：<用完整句写清根本原因，如"用户表中部分老用户的 password_salt 字段为空，hashUtil 未做空值判断">
- **问题类型**：回归缺陷（新代码导致旧功能失效）/ 功能缺失（应该有但没有的功能）/ 设计缺陷（架构或逻辑错误）

## TDD 修复计划

1. **RED**: 编写测试验证 [用完整句描述要测的行为，如"盐值为空时不抛 NPE 而是返回 401"]
   **GREEN**: [用完整句描述最小修改，如"在 hashUtil.verify 入口加盐值空值判断，抛 InvalidCredentialException"]

2. **RED**: 编写测试验证 [下一行为，如"盐值非空时正常校验通过"]
   **GREEN**: [最小变更使其通过]

**REFACTOR**: [全部测试通过后的清理工作，如"提取盐值校验为独立方法"，无则写"无"]
